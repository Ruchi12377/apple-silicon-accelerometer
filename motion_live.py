...
from spu_sensor import (
    sensor_worker, shm_read_new, shm_snap_read,
    SHM_NAME, SHM_NAME_GYRO, SHM_SIZE, GYRO_SCALE,
...
def render(det, t_start, restarts, lid_angle=None, als_raw=None):
    ...
    try:
        while running[0]:
            ...
            for idx, (sx, sy, sz) in enumerate(samples):
                ...
                det.process(sx, sy, sz, t_sample)

            gyro_samples, last_gyro_total = shm_read_new(
                shm_gyro.buf, last_gyro_total, scale=GYRO_SCALE)
            ...
            if now - last_draw >= 0.1:
                frame = render(det, t_start, restart_count[0],
                               lid_angle=lid_angle, als_raw=als_raw)
